...
public class RedBlackTree {
    ...
    public void insert(java.lang.String key, Transition value){
    ...
    //If key is in tree, returns the Transition value stored under that key
    // Pre-Post conditions:
        // Precondition: key value is contained with tree
        // Postcondition: Returns transition stored under key. Otherwise returns null if key is not found
    // Best case runtimes for method below:
        // Big-Theta: theta(1) => o(1), omega(1)
    // Worst case runtimes for method below:
        // Big-Theta: Theta(log(n)) => O(log(n)), Omega(log(n))
    public Transition lookup(java.lang.String key){
    ...
    public static void main(String[] args){
        ...
        for(char j = 'A'; j <= 'Z'; j++){
            System.out.println("key: " + j + ", value: 'Transition for char " + j + "'");
            rbt.insert(String.valueOf(j), new Transition(j,j,1,1));
        }

        // what value is stored under key = var16
        System.out.println("\n*** Key Search test ***");
        System.out.println("BigInteger value found under key \'D\': " + rbt.lookup("D"));

        // inserting duplicate key
        System.out.println("\n**** inserting duplicate keys (values should be replaced) ****");
        System.out.println("insert -> key: D, value: " + "H");
        rbt.insert("D", new Transition('H','H',1,1));

        // what value is stored under key = var16
        System.out.println("\nBigInteger value found under key \'D\': " + rbt.lookup("D"));
