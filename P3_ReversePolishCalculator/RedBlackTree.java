...
public class RedBlackTree {
    ...
    public RedBlackTree(){
        this.nil = new RedBlackNode(null, null, RedBlackNode.BLACK,null,null,null);
        this.root = this.nil;
    }
    // Pre-Post conditions:
    ...
    //places a key value into the tree, if key is not already stored
    // Pre-Post conditions:
    ...
    public void insert(java.lang.String key, BigInteger value){
        ...
        while (x != this.nil){
            y = x;
            int comparison = key.compareTo(x.getKey());
            if(comparison < 0){
                x = x.getLc();
            } else if(comparison == 0){   // if key is already stored in tree, replace value with one passed in and exit method
                x.setValue(value);
                return;  //no need to create new node or re-balance tree in this case. Exit method
            } else {
                x = x.getRc();
            }
        }
        RedBlackNode z = new RedBlackNode(key, value, RedBlackNode.RED, y, this.nil, this.nil);
    ...
    //If key is in tree, returns the BigInteger value stored under that key
    // Pre-Post conditions:
        // Precondition: key value is contained with tree
        // Postcondition: Returns BigInteger stored under key. Otherwise returns null if key is not found
    // Best case runtimes for method below:
        // Big-Theta: theta(1) => o(1), omega(1)
    // Worst case runtimes for method below:
        // Big-Theta: Theta(log(n)) => O(log(n)), Omega(log(n))
    public BigInteger lookup(java.lang.String key){
        ...
        while(x != this.nil){
            ...
            if(comp == 0){
                return x.getValue();  // if node key is equal to key passed in, return true
            } else if(comp < 0){
                x = x.getLc(); // if key value is less than node key value move to left child node
            } else{
                x = x.getRc(); // if key value is greater than node key value move to right child node
            }
        }
    ...
    // Pre-Post conditions:
    // Postcondition: Returns boolean answer to whether the tree contains key
    // Best case runtimes for method below:
    // Big-Theta: theta(1) => o(1), omega(1)
    // Worst case runtimes for method below:
    // Big-Theta: Theta(log(n)) => O(log(n)), Omega(log(n))
    public boolean contains(java.lang.String key){
        ...
        while(x != this.nil){
            ...
            int comp = key.compareTo(x.getKey());
            if(comp == 0){
                return true;  // if node value is equal to key return true
            } else if(comp < 0){
                x = x.getLc(); // if key value is less than node key value move to left child node
            } else{
                x = x.getRc(); // if key value is greater than node key value move to right child node
    ...
    //The method closeBy(key) returns a value close to key in the tree (most consecutive characters in common)
    // Pre-Post conditions:
        // Postcondition: Returns the value with the most number of characters in common with string key
    // Best case runtimes for method below: (will always run through entire tree because value isn't stored)
        // Big-Theta: Theta(log(n)) => O(log(n)), Omega(log(n))
    // Worst case runtimes for method below:
        // Big-Theta: Theta(log(n)) => O(log(n)), Omega(log(n))
    public java.lang.String closeBy(java.lang.String key){
        char[] vChars = key.toCharArray();   //store characters of serach term
        char[] closestValChars = this.root.getKey().toCharArray();  //saves chars of closest value found
        int matchingChars = 0;
        RedBlackNode x = this.root;  //creates node referance to be iterated through
        while(x != this.nil){
            char[] xChars = x.getKey().toCharArray();
            ...
            int comp = key.compareTo(x.getKey());
            if(comp == 0){
                return x.getKey();  // if node value is equal to key return key
            } else if(comp < 0){
    ...
    public static void main(String[] args){
        ...
        System.out.println("\n**** Key Value Storage RBTree Test ****");
        System.out.println("Insert the following key value pairs into tree");
        for(int j = 1; j <= 50; j++){
            System.out.println("key: var" + j + ", value: " + j);
            rbt.insert("var"+j, BigInteger.valueOf(j));
        }

        // what value is stored under key = var16
        System.out.println("\n*** Key Search test ***");
        System.out.println("BigInteger value found under key \'var16\': " + rbt.lookup("var16"));

        // inserting duplicate key
        System.out.println("\n**** inserting duplicate keys (values should be replaced) ****");
        System.out.println("insert -> key: var" + 16 + ", value: " + (20));
        rbt.insert("var"+16, BigInteger.valueOf(20));

        // what value is stored under key = var16
        System.out.println("\nBigInteger value found under key \'var16\': " + rbt.lookup("var16"));


        //a more in depth testing routine. please ignore
//        System.out.println("\n**** RBTree Test ****");
//        System.out.println("Insert the following key value pairs into tree");
//        for(int j = 1; j <= 5; j++){
//            System.out.println("key: " + j + ", value: " + j);
//            rbt.insert(""+j, BigInteger.valueOf(j));
//        }
//
//        // after 1..5 are inserted
//        System.out.println("\n**** Print RBT in order ****");
//        rbt.inOrderTraversal();
//        System.out.println("\n**** Print RBT in level order ****");
//        rbt.levelOrderTraversal();
//
//        // is 3 in the tree
//        System.out.println("*** Key Search test ***");
//        System.out.println("Does tree contain the key \'3\'?");
//        if(rbt.contains(""+3)) System.out.println("Found 3");
//        else System.out.println("No 3 found");
//        System.out.println("Value under key = 3: ");
//
//        // display the height
...
//        System.out.println("Insert the following key value pairs into tree");
//        for(int j = 1; j <= 5; j++){
//            System.out.println("key: " + j + ", value: " + (j*3));
//            rbt.insert(""+j, BigInteger.valueOf(j*3));
//        }
//
//        System.out.println("\n**** Print RBT in order ****");
//        rbt.inOrderTraversal();
